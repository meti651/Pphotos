...
const Slide = ({ image, slide }) => {
	return (
		<>
			...
			>
				<div id={Styles.background_image}>
					<Img fluid={image.image.childImageSharp.fluid} alt="background" />
				</div>
				<div className={Styles.container}>
					<div id={Styles.placeholder}></div>
					<div
						id={Styles.image_container}
						style={{ maxWidth: `${image.image.childImageSharp.fluid.aspectRatio * 60}%` }}
					>
						<Img fluid={image.image.childImageSharp.fluid} alt="slide" />
					</div>
					<div id={Styles.caption}>{image.caption}</div>
				</div>
			</div>
		</>
	);
